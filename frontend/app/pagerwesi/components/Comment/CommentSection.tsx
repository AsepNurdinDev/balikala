...
export default function CommentSection() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const pid = await getFestivalPostId("pagerwesi");
        setPostId(pid);
          ...
          <div className="lg:col-span-5 bg-stone-50 border border-stone-200/50 rounded-3xl p-6 shadow-soft">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="space-y-1.5">
                <label htmlFor="pagerwesi-name-input" className="block text-xs font-bold uppercase tracking-wider text-stone-500">
                  Nama Lengkap
                </label>
                <div className="relative">
                  <User className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-stone-400" />
                  <input
                    id="pagerwesi-name-input"
                    type="text"
                    required
                    value={name}
              ...
              {/* Comment Field */}
              <div className="space-y-1.5">
                <label htmlFor="pagerwesi-comment-input" className="block text-xs font-bold uppercase tracking-wider text-stone-500">
                  Komentar Anda
                </label>
                <textarea
                  id="pagerwesi-comment-input"
                  required
                  rows={4}
                  value={commentText}
